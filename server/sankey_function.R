...
sankey_gen <- function(html = FALSE){
  ...
  for (i in 1:length(paths)){
    if (i == 1){
      timepoints <- c(timepoints,rep(paths[i],total_num_list[i]))
    } else {
      timepoints <- c(timepoints,rep(paths[i],total_num_list[i]-total_num_list[i-1]))
    }
    
  }
  timepoints <- c(timepoints,rep(paths[i]+1,total_num_list[i+1]-total_num_list[i]))
  ...
  sankey_data <- sankey_data %>%
    ...
    mutate(probability = count / total_count, count = count) %>%
    select(-total_count)
  ...
  sankey_data$count <- as.numeric(sankey_data$count)
  ...
  sankey_data %>%
    group_by(PATHNO) %>%
    summarise(total_probability = sum(probability, na.rm = TRUE), total_counts = sum(count, na.rm = TRUE))
  ...
  # path_links
  num_paths2 = length(paths)
  
  for (i in 1:num_paths2){
    ## Gathering path_links and number for each path
    data_p <- data_sub %>% 
    ...
    if (i==1){
      ...
      path_links <- rbind(path_links, data_p)
    }
  }
  
  
  path_links <- path_links[order(path_links$PATHNO_ENCODED, path_links$value),] 
  
  # Ordering
  num_nodes = length(nodedata$node)
  ...
  path_2 <- path_links %>% 
    filter(PATHNO_ENCODED == paths[num_paths2]) %>%
  ...
  nodedata_ord$node_ord <- (1:num_nodes-1)
  ...
  if (isolate(input$order)){
    if (isolate(input$order_option == 'Name')){
      path_links <- path_links[order(path_links$PATHNO_ENCODED, path_links$NODE_S_ENCODED, path_links$NODE_E_ENCODED),]
    }
  }
  ...
  if (isolate(input$mode_switch)){
    link_group <- "ORIGIN2"     
    path_links$ORIGIN2 <- path_links$ORIGIN %>%
      str_replace_all(' ', '_')
    
    my_groups <- path_links$ORIGIN2 %>%
      unique() 
    ...
    path_links[,link_group] <- path_links[,paste0(isolate(input$node_s_e),'_ENCODED')] %>%
      str_replace_all(' ', '_')
    
    my_groups <- path_links[,link_group] %>%
      unique() 
    ...
    if (isolate(input$link_group)!="None" & isolate(input$link_group)!="none" ){
      link_col <- c()
      t_node_s <- grepl(isolate(input$link_group), path_links$NODE_S_ENCODED, ignore.case = TRUE)
      t_node_e <- grepl(isolate(input$link_group), path_links$NODE_E_ENCODED, ignore.case = TRUE)
      for (x in 1:length(path_links$NODE_S_ENCODED)){
  ...
  # Creates a variable for the unique path numbers from the data, then sorts it in ascending order 
  ...
  # For each unique path number, the unique starting nodes are pulled from our dataset.
  for (p in unique_path_no){
    ...
    # Appends the starting nodes of each path
    total_nodes <- total_nodes %>%
      append(node_temp)
    
    # Appends the cumulative total number of unique starting nodes after each path
    total_no_list <- total_no_list %>%
      append(length(total_nodes))
  }
  
  # Overwrites the node_temp variable to include names of unique end nodes on any path
  node_temp <- paste(unique(sankey_data_subset %>%
                              filter(PATHNO == p) %>%
                              pull(target)))
  ...
  # Creates a variable for the length of our path variable i.e. number of unique unique_path_no
  no_of_paths2 <- length(unique_path_no)
  
  # For each path, the data is grouped by the path combo (start and end nodes) and the probability associated with that combination 
  for (i in 1:no_of_paths2){
    ...
    sankey_data_p <- sankey_data_subset %>% 
      ...
      group_by(source, target, probability,count) %>%
      summarise(PATHNO = mean(PATHNO))
    
    # If it is the first path, it merges the first probability of node_data (number of nodes for first path) sankey_data_p variable
    # and creates by variables for names of starting and ending nodes then assigns this to a links variable.
    if (i == 1){
      sankey_data_p <- sankey_data_p %>%
        merge(node_data[1:total_no_list[1],], by.x = "source", by.y = "name", all = TRUE) %>%
        merge(node_data[(total_no_list[1] + 1):total_no_list[2],], by.x = "target", by.y = "name", all = TRUE)
      links <- sankey_data_p
    }
    # If it is not the first path, it instead creates the variables for names of starting and ending nodes for that path and adds
    # it to the links variable.
    else {
      sankey_data_p <- sankey_data_p %>%
        merge(node_data[(total_no_list[i - 1] + 1):total_no_list[i],], by.x = "source", by.y = "name", all = TRUE) %>%
        merge(node_data[(total_no_list[i] + 1):total_no_list[i + 1],], by.x = "target", by.y = "name", all = TRUE)
      
      links <- rbind(links, sankey_data_p)
    }
  }
  
  
  # Orders the links variable we just created by the path number and probability
  links <- links[order(links$PATHNO, links$probability),] 
  ...
  no_nodes <- length(node_data$node)
  
  # Will contain all the nodes for each path ordered by path and then by size
  # New data frame variable that is an empty version of node_data
  node_data_ord <- node_data[0,]
  
  # For each path, it adds the starting node and size to the path variable
  for (i in 1:no_of_paths2){
    # Getting the size of each node
    path <- links %>% 
      filter(PATHNO == unique_path_no[i]) %>%
    ...
    # For each path it also makes a variable for the node number for each node
    node_data_1 <- NULL
  ...
  # The path variable is overwritten by one where it filters for the final path number, and gives it the probability of 
  ...
  node_data_ord$node_ord <- (1:no_nodes - 1)
  
  # For each path, the data is grouped by the unique_path_no and the number of each path is given as well as the path number
  for (i in 1:no_of_paths2){
    sankey_data_p <- sankey_data_subset %>%
      filter(PATHNO == unique_path_no[i]) %>%
      group_by(source, target, probability, count) %>%
    ...
    if (i == 1){
      sankey_data_p <- sankey_data_p %>%
        merge(node_data_ord[1:total_no_list[1],], by.x = "source", by.y = "name", all = TRUE) %>%
        merge(node_data_ord[(total_no_list[1] + 1):total_no_list[2],], by.x = "target", by.y = "name", all = TRUE)
    ...
    # it to the links variable.
      ...
      sankey_data_p <- sankey_data_p %>%
        merge(node_data_ord[(total_no_list[i - 1] + 1):total_no_list[i],], by.x = "source", by.y = "name", all = TRUE) %>%
        merge(node_data_ord[(total_no_list[i] + 1):total_no_list[i + 1],], by.x = "target", by.y = "name", all = TRUE)
      
      links <- rbind(links, sankey_data_p)
    }
  }
  
  # Orders the links variable by the path number and its probability (number of those path)
  links <- links[order(links$PATHNO, links$probability),]
  ...
  node_probabilities <- sankey_data %>%
    select(source, target, probability, count, PATHNO) %>%
    arrange(PATHNO, source, target) %>%
    group_by(PATHNO, source, target) %>%
    summarize(total_probability = sum(probability), total_count = sum(count)) %>%
    distinct()
  ...
  node_data_ord2 <- node_data_ord %>%
    mutate(target = node_probabilities$target,
           ...
           count = node_probabilities$total_count,
  ...
  node_data_ord2 <- node_data_ord2 %>%
    mutate(name_prob = paste0(name, " : ", count, " (", round(probability * 100, 2), "%)"))
  ...
  links <- left_join(links, path_links, 
  ...
  nodedata_ord <- left_join(node_data_ord2, nodedata_ord, 
                            by=c('node', 
                                 'name',
                                 'node_ord',
                                 'count'='size'))
  ...
  sankey$x$path_links$ORIGIN <- path_links$ORIGIN
  sankey$x$path_links$PATHNO_ENCODED <- path_links$PATHNO_ENCODED
  ...
  # print(links)
  # print(path_links)
  # print(node_data_ord2)
  # print(nodedata_ord)
  # print(nrow(links))
  # print(nrow(path_links))
  # print(nrow(node_data_ord2))
  # print(nrow(nodedata_ord))
  # print(link_data)
  # print(node_data_ordered)
  
  sankey <- onRender(sankey,js_code)
  
  #sankey <- appendContent(sankey,js_code)
